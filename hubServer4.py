...
# TODO:
# UTC to local timestamps
# if DB connection temporarily fails in the middle of the program, keep trying to reconnect **
...
def does_device_exit(): 
    ...
    try:    
        ...
        dprint (reqj)
        existance = db.deviceExists (reqj)  # this returns a boolean
        retval = {'device_exists' : existance}
    except Exception as e:
        retval = {'error' : str(e)} 
    dprint (retval)
    return (retval)  
...
#if not db.connect ():
if not db.connect (db=PasswordStore.db_name, host=PasswordStore.db_host, port=PasswordStore.db_port):
